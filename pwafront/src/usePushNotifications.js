...
export default function usePushNotifications() {
  ...
  const onClickSendSubscriptionToPushServer = () => {
    ...
    setError(false);
    http
      .post("/subscription", {subscriptionRequest: userSubscription, token : localStorage.getItem('token')})
  ...
  const onClickSendNotification = async () => {
    setLoading(true);
    setError(false);
    await http.post(`/subscription/${pushServerSubscriptionId}`, {title : 'jihad', token : localStorage.getItem('token')}).catch(err => {
      setLoading(false);
      setError(err);
    });
    setLoading(false);
